...
class PagosController {
  ...
  async getPagosCliente(idContrato) {
    ...
    async proyeccionEspecifica(mes, anho) {
        try {
            let resultSoles = await PagosDb.calcularProyecciónIntervaloSolesEspecifico(mes, anho);
    ...
    async proyeccionIntervalo(FechaInicio, FechaFin) {
        try {
            ...
            let resultDolares = await PagosDb.calcularProyecciónIntervaloDolaresIntervalo(FechaInicio, FechaFin);
            if (resultDolares.Dolares == null) { resultDolares.Dolares = 0; }
    ...
    async proyeccionIntervaloMESES_PROBLEMATICOS(mesInicial, anhoInicial, mesFinal, anhoFinal) {
        try {
            ...
            if (prueba.Soles == null) { resultSoles.Soles = 0; }
            let resultDolares = await PagosDb.calcularProyecciónIntervaloDolaresIntervaloProblematico(fechaInicial, FechaFin);
            if (resultDolares.Dolares == null) { resultDolares.Dolares = 0; }
    ...
    async update(idPago, idEstado) {
        ...
        return PagosDb.actualizarPagoCliente(idPago, idEstado)
            .catch((err) => {
                console.log("Controller Error: ", err);
                return null;
            });
    ...
    async createObjects(idLote, coutasIniciales, coutasFinanciadas) {
        try {
            for (const coutaInicial of coutasIniciales) {
                ...
                await PagosDb.createPagoInicial(idLote, monto, fecha, saldo);
            }
